...
* An application may join one or more `container` mode networks. If joining multiple networks, `networkName` must be specified in each `portMapping` declaration that specifies a `hostPort` value. Note that in this scenario, each container network must be explicitly named; a `portMapping` cannot refer to a container network by the name it would receive via the `--default_network_name` command-line flag.
