    ...
    networks.toIterator
      ...
      .map { network =>
        ...
        val portMappings = container.portMappings.withFilter(_.hostPort.nonEmpty).withFilter { mapping =>
          // if hostPort is specified, networkName is required when more than one container networks are defined.
          // If networkName is not specified, then there is only one network
